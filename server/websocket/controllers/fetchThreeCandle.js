...
const fetchThreeCandle = async (insToken) => {
  ...
  data.forEach((tick) => {
    ...
    const roundedIST = istTime
      .startOf("minute")
      .minus({ minutes: istTime.minute % 3 }).ts;
  ...
  const candle = new threeCandleData({
  ...
  await candle.save();
  console.log("Data saved 3 Min: ", candle);
  //   return candles;
};

// fetchFiveCandle([9372674]);
module.exports = { fetchThreeCandle };
